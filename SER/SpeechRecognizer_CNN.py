...
#   https://github.com/maelfabien/Multimodal-Emotion-Recognition/blob/master/01-Audio/Notebook/CNN-LSTM/01%20-%20Preprocessing%20%5BCNN-LSTM%5D.ipynb
#
#####################
...
# RAVDESS Database
label_dict_ravdess = {'01': 'NEU','02':'CAL', '03': 'HAP', '04': 'SAD', '05': 'ANG', '06': 'FEA', '07': 'DIS', '08': 'SUR'}


# Set audio files labels
def set_label_ravdess(audio_file, gender_differentiation):
    label = label_dict_ravdess.get(audio_file[20:22])
    if gender_differentiation == True:
        if int(audio_file[32:34]) % 2 == 0:     # Female
            label = 'f_' + label
        if int(audio_file[32:-34]) % 2 == 1:     # Male
            label = 'm_' + label
    return label
...
for audio_index, audio_file in enumerate(glob.glob("data//Actor_*//*.wav")):

    if audio_file[20:22] in list(label_dict_ravdess.keys()):
...
random_filename = 'data\Actor_01\03-01-02-02-01-01-01.wav'
...
pickle.dump(X_train.astype(np.float16), open('data//Model//[RAVDESS][MEL_SPECT][X_train].p', 'wb'))
pickle.dump(y_train, open('data//Model/[RAVDESS][MEL_SPECT][y_train].p', 'wb'))
pickle.dump(X_test.astype(np.float16), open('data/Model/[RAVDESS][MEL_SPECT][X_test].p', 'wb'))
pickle.dump(y_test, open('data/Model/[RAVDESS][MEL_SPECT][y_test].p', 'wb'))
